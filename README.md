...
- 📱 **二维码**：生成结果自带二维码，可一键下载 PNG，扫码即可在手机打开。
- 📊 **访问统计**：汇总访问次数、短链数量、最近创建时间，可视化「近 7 天新增短链」柱状图与「访问量 TOP」排行。
- 🧠 **管理后台**：短链列表支持搜索、排序、一键删除与分页加载；统计图表一屏掌握，`?view=stats` 可直达统计视图。
- 🧭 **品牌化错误页**：短链不存在或已失效时展示统一风格的引导页，而非纯文本报错。
...
4. 若配置了 `ADMIN_PATH`，通过侧边栏「短链列表 / 访问统计」直达管理后台对应视图：
   - **短链列表**：查看全部短链、原始链接、访问次数与创建时间（悬停显示完整时分秒），支持搜索、排序、删除与分页加载。
   - **访问统计**：总访问量 / 短链总数 / 最近创建，以及近 7 天新增与访问量 TOP 排行。
5. 生成结果支持下载短链**二维码**（PNG），扫码即可在手机上打开。
...
├── pages.js            # 四个页面模板（登录 / 主页 / 管理后台 / 错误页，统一设计系统，仅此一处维护 UI）
├── utils.js            # 公共工具函数
