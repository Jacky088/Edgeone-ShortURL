...
基于 **腾讯云 EdgeOne Pages** 构建的无服务器短链接服务（URL Shortener）。支持快速创建短链接、访问统计与简易管理后台；自带全新蓝色科技风 UI，原生适配**桌面端 / 移动端**与**日间 / 夜间模式**，适合部署在 EdgeOne 平台并绑定自定义域名使用。
...
- 🚀 **无服务器架构**：基于 EdgeOne Pages Functions + KV，低延迟、几乎零运维。
- ⚙️ **简单部署**：GitHub 一键部署，无需额外服务器。
