...
// 公共的 CSS 变量、顶栏样式、主题切换与粒子动画脚本在此只维护一份，
// 由 buildPage() 组装；页面私有内容通过参数注入。

// ==========================================
// 共享图标数据 (Base64/DataURI)
// ==========================================
...
function baseCss(overflow) {
  return `        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: var(--bg-color); color: var(--text-color); margin: 0; display: flex; justify-content: center; align-items: center; min-height: 100vh; padding: 1rem; box-sizing: border-box; overflow: ${overflow}; }
        #particle-canvas { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; pointer-events: none; }
`;
}

function topBarCss(extra = '') {
  return `        .top-bar { position: fixed; top: 1rem; right: 1rem; display: flex; gap: 0.5rem; align-items: center; z-index: 10; }
...
// 共享 HTML 片段（顶栏）
// ==========================================
function topBarHtml({ logout = false, backHome = false, adminLink = false } = {}) {
  return `<div class="top-bar">
${logout ? '    <button type="button" id="logout-btn" class="text-btn">注销</button>\n' : ''}${adminLink ? '    <a href="javascript:void(0)" id="admin-link" class="text-btn">管理后台</a>\n' : ''}${backHome ? '    <a href="/" class="text-btn">返回前台</a>\n' : ''}    <a href="https://github.com/Jacky088/Edgeone-ShortURL" target="_blank" class="icon-btn" title="Jacky088/Edgeone-ShortURL">
...
// 共享脚本片段（主题切换 + 粒子背景）
// ==========================================
const themeParticleJs = `    // UI & Theme
    ...
    function setTheme(theme) { htmlEl.setAttribute('data-theme', theme); localStorage.setItem('theme', theme); if (theme === 'dark') { iconSun.style.display = 'block'; iconMoon.style.display = 'none'; } else { iconSun.style.display = 'none'; iconMoon.style.display = 'block'; } if (typeof window.initParticles === 'function') window.initParticles(); }
...
const initThemeJs = `    const storedTheme = localStorage.getItem('theme'); if (storedTheme) setTheme(storedTheme); else setTheme('light'); window.initParticles();`;
...
`,
  script: themeParticleJs + adminLinkJs + `
    // Auth Logic
...
` + topBarCss(),
  body: topBarHtml({ logout: true, adminLink: true }) + `
<div class="container">
    <h1>短链接生成服务</h1>
    <form id="link-form">
        <div class="form-main">
...
`,
  script: themeParticleJs + adminLinkJs + logoutJs + `
    // Link Logic
...
` + topBarCss(`        @media (max-width: 600px) { th:nth-child(2), td:nth-child(2) { display: none; } }\n`),
  body: topBarHtml({ logout: true, backHome: true }) + `
<div class="container">
    <h1>面板数据</h1>
...
`,
  script: themeParticleJs + logoutJs + `
    // Admin Logic
